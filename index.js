  ...
  L.VegaLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    initialize(spec, options) {
      ...
      if (!vega.expressionFunction("setMapView")) {
        vega.expressionFunction("setMapView", function setMapView(...args) {
          const view = this.context.dataflow;
          const handler = view.Leaflet_setMapViewHandler;
          if (!handler)
            throw new Error("setMapView() is not defined for this graph");
    ...
    async _onAddAsync(map) {
      ...
      try {
        ...
        const dataflow = vega.parse(
          this._spec,
          this.options.parseConfig,
          ...
          switch (args.length) {
            ...
            case 1: {
              ...
              if (
                ...
                map.fitBounds(
                  L.latLngBounds(L.latLng(lat1, lng1), L.latLng(lat2, lng2))
                );
              } else {
